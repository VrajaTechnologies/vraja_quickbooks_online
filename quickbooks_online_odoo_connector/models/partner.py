...
class ResPartner(models.Model):
    ...
    qbk_customer_id = fields.Char("QuickBooks Customer ID")
    ...
    def export_to_quickbooks(self):
        for partner in self:
            if partner.customer_rank > 0:
                endpoint = "customer"
                if partner.qbk_id:
                    msg_body = f"{partner.name} already exists in QuickBooks as {endpoint} with ID: {partner.qbk_id}."
                    partner.message_post(body=msg_body)
                    continue
            elif partner.supplier_rank > 0:
                endpoint = 'vendor'
                if partner.qbk_vendor_id:
                    msg_body = f"{partner.name} already exists in QuickBooks as {endpoint} with ID: {partner.qbk_vendor_id}."
                    partner.message_post(body=msg_body)
                    continue
            else:
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if endpoint == "customer":
                            partner.qbk_id = qbk_id
                        else:
                            partner.qbk_vendor_id = qbk_id
